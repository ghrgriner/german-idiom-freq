#    Count regular expresion combinations in corpora using MPI.
...
'''Count regular expression combinations in corpora using MPI.

This program is like `count_regexes.py`, except parallelization is done using
MPI (mpi4py.futures) instead of the `multiprocessing` package.
...
def mpi_count_regexes(df, output_file, chunksize, verb_forms=None,
                  n_cores=None,
                  line_generator=None,
    ...
    verb_forms : Dict[str, str]
        ...
        this convention.
    n_cores : None
        Not used. Must be `None`. The number of workers is determined
        automatically by MPIPoolExecutor.
    ...
    if line_generator is None and corpus_files is None:
        raise ValueError('mpi_count_regexes: `line_generator` or '
    ...
    if df.headword.duplicated().any():
        ...
        raise ValueError('mpi_count_regexes: Duplicate headwords in `df`')
